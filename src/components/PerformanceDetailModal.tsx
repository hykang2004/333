...
import { X, Clock, MapPin, Users, Award } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      ...
      >
        ...
        <h3 className="text-xl sm:text-2xl font-bold text-white tracking-tight mb-1 font-serif-magic">
          {isKo ? performance.titleKo : performance.titleEn}
        </h3>
        {((isKo ? performance.clubNameKo : performance.clubNameEn) || '').trim() && (
          <p className="text-xs sm:text-sm font-medium text-amber-400 mb-4 flex items-center gap-1.5">
            <Award className="w-3.5 h-3.5" />
            <span>{isKo ? performance.clubNameKo : performance.clubNameEn}</span>
          </p>
        )}
        ...
        <div className="flex items-center gap-2 text-xs text-slate-300 bg-[#090d18] p-3 rounded-xl border border-white/5 mb-4">
          ...
          <span>{isKo ? performance.locationKo : performance.locationEn}</span>
        </div>

        {/* Description */}
        <div className="space-y-1.5 mb-4">
          <h4 className="text-xs font-semibold text-slate-400">
            {isKo ? '공연 소개' : 'About Show'}
          </h4>
          <p className="text-xs sm:text-sm text-slate-200 leading-relaxed bg-[#090d18] p-3.5 rounded-xl border border-white/5 font-light">
            {isKo ? performance.descriptionKo : performance.descriptionEn}
          </p>
        </div>

        {/* Performer Info */}
        {performance.performerInfoKo && (
          <div className="space-y-1.5 mb-4">
            <h4 className="text-xs font-semibold text-slate-300 flex items-center gap-1.5">
              <Users className="w-3.5 h-3.5 text-amber-400" />
              <span>{isKo ? '출연진 & 세션 구성' : 'Performers'}</span>
            </h4>
            <p className="text-xs text-slate-300 bg-[#090d18] p-3 rounded-lg border border-white/5">
              {isKo ? performance.performerInfoKo : performance.performerInfoEn}
            </p>
          </div>
        )}
